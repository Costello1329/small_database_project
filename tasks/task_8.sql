...
INSERT INTO motoservice._worker_ (WORKER_ID, WORKER_NM, WORKER_BIRTHDAY, WORKER_INCOME, DEPARTMENT_ID) VALUES
  ...
  (10, 'Sodikov Memedequeue', '2000-01-20', 150200, 1);
...
CREATE OR REPLACE FUNCTION motoservice.concat_trig_func()
  RETURNS TRIGGER AS
  $$
    BEGIN
      RAISE NOTICE '% is deleted', OLD.CLIENT_NM;
      RETURN OLD;
    END
  $$
  LANGUAGE PLPGSQL;
...
CREATE TRIGGER update_it
  BEFORE DELETE ON motoservice._client_
  FOR EACH ROW
  EXECUTE PROCEDURE motoservice.concat_trig_func();
